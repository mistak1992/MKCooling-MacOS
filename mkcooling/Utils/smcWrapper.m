...
+(int)convertToNumber:(SMCVal_t) val
{
    ...
    if (strcmp(val.dataType, DATATYPE_FLT) == 0 && val.dataSize == 4) {
        ...
        DDLogError(@"%@", [NSString stringWithFormat:@"Unknown val:%s size-%d",val.dataType,val.dataSize]);
    }
...
+ (BOOL)validateSMC:(NSString*)path
{
    ...
    @try {
        ...
        if (status != noErr) {
            DDLogError(@"Codesign verification failed: Error id = %d",status);
            return false;
        }
    }
    @catch (NSException *exception) {
        DDLogError(@"Codesign exception %@",exception);
        return false;
    }
...
+(void)setKey_external:(NSString *)key value:(NSString *)value{
    ...
    if (![checksum  isEqualToString:smc_checksum]) {
        DDLogError(@"smcFanControl: Security Error: smc-binary is not the distributed one");
        return;
    }
